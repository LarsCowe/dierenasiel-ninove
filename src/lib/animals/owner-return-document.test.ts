...
// Enkel de scan-tests vervangen `fetch`: @react-pdf laadt zijn wasm-layoutmotor zelf via
// fetch, en met een stub ervoor rendert er niets meer.
function metScan() {
  fetchMock.mockReset();
...
describe("buildOwnerReturnAttachment", () => {
  ...
  it("haalt de getekende scan op en houdt de extensie van de scan (een foto blijft een foto)", async () => {
    ...
    const bijlage = await buildOwnerReturnAttachment({
      ...form,
      signedDocumentUrl: "https://blob.vercel-storage.com/owner-return-forms/TNE-2026-0003-1-getekend.JPG?x=1",
    });
  ...
  it("valt terug op .pdf als de scan-URL geen extensie draagt", async () => {
    ...
    const bijlage = await buildOwnerReturnAttachment({ ...form, signedDocumentUrl: "https://blob/scan" });
  ...
  it("gooit een fout als de scan niet op te halen is, zodat de mail NIET zonder bijlage vertrekt", async () => {
    ...
    await expect(buildOwnerReturnAttachment({ ...form, signedDocumentUrl: "https://blob/weg.pdf" })).rejects.toThrow("404");
  });
});
